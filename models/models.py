...
class PeftModelFromCLIP(nn.Module):
    def __init__(self, cfg, clip_model, num_classes):
        ...
        # self.unlabeled_head = eval(cfg.classifier)(feat_dim, num_classes, dtype, **cfg)
    ...
    def forward(self, image, use_tuner=True, return_feature=False, unlabeled=False):
        tuner = self.tuner if use_tuner else None
        head = self.head if not return_feature else None
        # if unlabeled:
        #     head = self.unlabeled_head
